...
/**
 * RTM Standard Seed File - Enhanced with Standardization Features (FIXED)
 * 
 ...
 * - FIXED: Proper handling of existing data and foreign key constraints
 * 
 * Version: 3.1 - Fixed Standardization Features
 * Last Updated: 2024
 */
...
// ===== ENHANCED SEED FUNCTION WITH PROPER DATA HANDLING =====
async function seedRTMStandardFixed() {
  console.log('🌱 Starting Enhanced RTM Standard Seed with Standardization (FIXED)...');
  ...
  try {
    ...
    await safeDelete('timeLog', 'deleteMany');
    ...
    // Create/Update metric definitions using upsert
    ...
    for (const template of assessmentTemplates) {
      ...
      await prisma.assessmentTemplateItem.deleteMany({
        where: { assessmentTemplateId: created.id }
      });
      ...
      if (template.items && template.items.length > 0) {
        for (const item of template.items) {
          ...
          if (metric) {
            ...
            console.log(`    ✅ Linked ${template.name} → ${item.metricKey}`);
          } else {
            console.log(`    ⚠️  Metric ${item.metricKey} not found for template ${template.name}`);
          }
        }
      }
    }
    ...
    console.log('✅ Enhanced RTM Standard seed with standardization completed successfully!');
    console.log(`📊 Created/updated ${Object.keys(createdMetrics).length} metric definitions`);
    console.log(`📋 Created/updated ${Object.keys(createdTemplates).length} standardized assessment templates`);
    console.log(`🏥 Created/updated ${Object.keys(createdPresets).length} standardized condition presets`);
    console.log(`🔗 Created assessment template item linkages for metric associations`);
