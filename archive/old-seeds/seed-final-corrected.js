...
// Standardized metrics with correct enum values
...
async function seedFinalCorrected() {
  try {
    ...
    // Clean in correct order (respecting foreign key constraints)
    ...
    for (const preset of conditionPresets) {
      ...
      // For ConditionPresetDiagnosis, since there are no unique constraints,
      // we need to check if they exist first, then create if they don't
      for (const diagnosis of preset.diagnoses) {
        const existingDiagnosis = await prisma.conditionPresetDiagnosis.findFirst({
          where: {
    ...
    for (const template of assessmentTemplates) {
      ...
      // Create template items (this has the correct unique constraint)
