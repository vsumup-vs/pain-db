...
async function safeDelete(modelName, tableName) {
  try {
    ...
    console.log(`✅ Cleared ${result.count} records from ${tableName}`);
  } catch (error) {
    if (error.code === 'P2003') {
      console.log(`⚠️  Cannot clear ${tableName} due to foreign key constraints`);
    } else {
...
const standardizedMetrics = [
  ...
  },
  {
    ...
    validation: {
      ...
      stage1Hypertension: { min: 130, max: 139 },
      stage2Hypertension: { min: 140, max: 180 },
      hypertensiveCrisis: { min: 180, max: 250 }
    }
  },
  {
    key: 'diastolic_bp',
    ...
    validation: {
      ...
      stage1Hypertension: { min: 80, max: 89 },
      stage2Hypertension: { min: 90, max: 120 },
      hypertensiveCrisis: { min: 120, max: 150 }
    }
  },
  {
    key: 'weight',
...
async function seedRTMStandardFinalFixed() {
  try {
    console.log('🚀 Starting RTM Standard Data Seeding (Final Fixed Version)...');
    const now = new Date();
    ...
    for (const metric of standardizedMetrics) {
      const created = await prisma.MetricDefinition.upsert({
        ...
        create: {
          id: generateUUID(),
          key: metric.key,
          name: metric.displayName, // Map displayName to name
          displayName: metric.displayName, // Add the missing displayName field
          displayName: metric.displayName, // Add the missing displayName field
          description: metric.description || `${metric.displayName} measurement`,
          unit: metric.unit,
          valueType: metric.valueType,
    ...
    // Create/update condition presets
    ...
    for (const preset of conditionPresets) {
      ...
      let existingPreset = await prisma.ConditionPreset.findFirst({
      ...
      if (existingPreset) {
        ...
        created = await prisma.ConditionPreset.update({
        ...
        // Create new preset
        created = await prisma.ConditionPreset.create({
          data: {
            id: generateUUID(),
            name: preset.name,
            defaultProtocolId: preset.defaultProtocolId,
    ...
    // Create/update assessment templates
    console.log('📋 Creating/updating assessment templates...');
    const createdTemplates = {};
    for (const template of assessmentTemplates) {
      // Check if template already exists by name
      let existingTemplate = await prisma.AssessmentTemplate.findFirst({
      ...
      if (existingTemplate) {
        ...
        created = await prisma.AssessmentTemplate.update({
          where: { id: existingTemplate.id },
          data: {
            description: template.description,
            questions: {}, // Required field, using empty object
        ...
        // Create new template
        created = await prisma.AssessmentTemplate.create({
          data: {
            id: generateUUID(),
            name: template.name,
            description: template.description,
      ...
      if (template.items && template.items.length > 0) {
        for (const item of template.items) {
          ...
          if (metric) {
            ...
            console.warn(`⚠️  Metric not found for key: ${item.metricKey}`);
          }
        }
        console.log(`  ✅ Created ${template.items.length} items for ${template.name}`);
      }
    }
    ...
    for (const link of presetTemplateLinks) {
      ...
      if (preset && template) {
        ...
        console.warn(`⚠️  Could not link ${link.preset} to ${link.template} - one or both not found`);
      }
    }

    console.log('🎉 RTM Standard Data Seeding completed successfully!');
