...
async function createMigration() {
  ...
  try {
    // Generate migration
    const { stdout, stderr } = await execAsync('npx prisma migrate dev --name add-assessment-template-items --create-only');
    console.log('✅ Migration created successfully');
    if (stdout) console.log(stdout);
    if (stderr) console.log('Warnings:', stderr);
    
    // Apply migration
    const { stdout: applyStdout, stderr: applyStderr } = await execAsync('npx prisma migrate dev');
    console.log('✅ Migration applied successfully');
    if (applyStdout) console.log(applyStdout);
    if (applyStderr) console.log('Warnings:', applyStderr);
    
    return true;
  } catch (error) {
    console.error('❌ Error creating/applying migration:', error.message);
    return false;
  }
}
...
async function main() {
  console.log('🚀 Fixing Assessment Template Items...\n');
  
  try {
    // Step 1: Add the model to schema
    const schemaUpdated = await addAssessmentTemplateItemModel();
    
    // Step 2: Create and apply migration (only if schema was updated)
    if (schemaUpdated) {
      const migrationSuccess = await createMigration();
      if (!migrationSuccess) {
